...
% 0.a Path to directories 
  ...
  root_dir = '/lustre/f2/dev/gfdl/Brandon.Reichl/Work/WW3/Model_Runs/Global_JRA/src/gridgen/';
  bin_dir = [root_dir '/bin/'];             % matlab scripts location
  ref_dir = [root_dir '/reference_data/'];  % reference data location
...
% 0.b Design grid parameters
  ...
  fname = 'glo_4deg';                  % file name prefix 
  
  grid_box = [-88 2 88 358];        % starting and ending lat,lon for grid 
                                     % domain. Longitude data is always
                                     % defined from 0 to 360
  dx = 4.0;                          % grid resolution in x (degrees)
  dy = 4.0;                          % grid resolution in y (degrees)
...
%    so the last x point is omitted
  ...
  write_ww3meta([out_dir,'/',fname],GTYPE,lon,lat,1/depth_scale,...
                                  1/obstr_scale);
